let _ =
      try
        ...
        let listHelper (name, precondsList, effectsList) = 
          ...
          let precondsListFinal = String.concat " * " (List.map parens precondsList1) in

          let effectsList1 = List.map merger effectsList in
          let effectsListFinal = String.concat " * " (List.map parens effectsList1) in

          let res = "fof(" ^ name ^ ", axiom, (" ^ precondsListFinal ^ ") -o (" ^ effectsListFinal ^ "))"in
